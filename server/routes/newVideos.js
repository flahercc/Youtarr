...
function createNewVideoRoutes({ verifyToken, newVideoQueueModule, newVideoScanScheduler }) {
   ...
   *     description: Videos discovered across all enabled/subscribed channels that are not yet downloaded or ignored, newest-discovered first.
   *     tags: [NewVideos]
   *     responses:
   *       200:
   ...
   * /api/new-videos/scan:
   *   post:
   *     summary: Scan all enabled channels for new videos
   *     description: Manually trigger the same scan the scheduled channel-scan task runs, refreshing every enabled channel's auto-download-enabled tabs.
   *     tags: [NewVideos]
   *     responses:
   *       200:
  ...
  router.post('/api/new-videos/scan', verifyToken, async (req, res) => {
    try {
      ...
      if (error.message === 'SCAN_IN_PROGRESS') {
        return res.status(409).json({ error: 'A channel scan is already in progress' });
      }
      logger.error({ err: error }, 'Failed to run channel scan');
      res.status(500).json({ error: 'Failed to run channel scan' });
    }
  });
