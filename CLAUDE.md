...
- `routes/`: API handlers (auth, channels, videos, videoDetail, videoSearch, channelSearch, config, jobs, plex, setup, subscriptions, apikeys, ytdlpOptions, health, maintenance, playlists, mediaServers, subfolders, newVideos). All use the dependency injection factory pattern; wiring lives in `server/routes/index.js`. `channels.js` also owns the single-video-download route (`POST /api/channels/:channelId/videos/:youtubeId/download`), the per-video counterpart to `download-all`, used by the new-videos queue.
...
- `modules/channel/`: channel domain decomposed from the former 2,851-line channelModule (`fetchRegistry` shared in-flight-fetch guard, `channelYtdlpExecutor` yt-dlp listing runner, `channelIdentity`, `tabState`, `channelMappers`, `videoEntryParser`, `channelVideoWriter`/`channelVideoQuery` channel-video persistence and querying, `channelThumbnails` (avatar + banner caching, poster/backdrop backfill via `backfillChannelImages`), `channelBackdropBackfill` (on `writeBackdropImages` enable: fetches missing channel banners and backfills backdrop.jpg into channel folders), `apiFallbackLogger`, `channelMetadataFetcher`, `tabManager`, `channelProvisioning` (getChannelInfo/upsertChannel), `channelCatalog` (list/enable/disable), `autoDownloadScheduler` (cron + channels.txt generation), `channelVideoFetcher`, `channelVideosService` (getChannelVideos/fetchAllChannelVideos; getChannelVideos' own auto-refresh-on-page-visit fetch also caps at `channelScanVideoLimit`, matching the scheduled scan, so viewing a freshly-added channel's page can't seed more undownloaded rows into the New Videos queue than the configured limit), `newVideoScanScheduler` (config-driven daily cron: `channelScanEnabled`/`channelScanTime` ("HH:MM" server-local, converted to a cron expression), reschedules on config change like `autoDownloadScheduler`/`watchStatusScheduler`; `scanAll()` is the primary entrypoint (cron tick and the manual `POST /api/new-videos/scan` trigger both call it), running channel scanning then playlist scanning under one in-progress guard - `scanAllChannels()` remains as a standalone, separately-guarded channel-only counterpart for focused reuse/testing. Channel scanning refreshes every enabled channel's `auto_download_enabled_tabs` via the existing `channelVideoFetcher` fetch/insert pipeline, capping each channel/tab fetch at `channelScanVideoLimit` (default 50, falling back to `channelVideoFetcher.DEFAULT_MAX_VIDEO_COUNT` when unset) via a `maxVideoCount` param threaded through `fetchChannelVideos`/`fetchAndSaveVideosViaYtDlp`, isolating per-channel/tab failures so one bad channel can't sink the scan. Playlist scanning refreshes every enabled playlist via `playlistModule.fetchAllPlaylistVideos` (always a full re-list; there's no incremental path), gated by a 1-hour `Playlist.lastFetched` freshness check mirroring the channel tabs' gate, bypassed by the manual trigger's `force`; a concurrent fetch of the same playlist (`FETCH_IN_PROGRESS`, e.g. from a page visit) is treated as a skip, not a failure), `newVideoQueueModule` (`getQueue()`: the cross-channel, cross-playlist "new and not yet downloaded or ignored" review list behind `GET /api/new-videos`, merging `getChannelCandidates()` (reusing `channelDownloadAllModule`'s eligibility rule - availability/live_status) and `getPlaylistCandidates()` (no separate eligibility filter needed: `playlistModule` already excludes unavailable/private/deleted entries and prunes removed ones before persisting `PlaylistVideo` rows), each row tagged `source: 'channel'|'playlist'` with `source_id`/`source_title`, sorted newest-first by first-seen time - `ChannelVideo.first_seen_at` for channel rows, `PlaylistVideo.added_at` for playlist rows (reliable as a first-seen timestamp only while undownloaded; it's re-stamped to the real download time afterward)); `channelModule.js` remains the public facade. `channelDownloadAllModule.js` (top-level `modules/`) additionally exposes `downloadSingleVideo(channelId, youtubeId)`, the single-video counterpart to `startDownloadAll`, used to queue an individual video from the new-videos queue.
...
- `components/shared/`: reusable components used across multiple features (e.g. `VideoModal/` for the video detail modal, `ThumbnailClickOverlay` for clickable thumbnail hotspots, `DeleteVideosDialog`, `AddChannelDialog` for confirming a channel add from search results or an unsubscribed channel name click). `VideoList/VideoListSelectionPill.tsx` renders the bulk-selection Download/Ignore/Delete actions as an icon-only floating button (bottom-right on desktop) rather than an inline labeled button like the New Videos Queue's "Download Selected" — tracked as a discoverability enhancement: [flahercc/Youtarr#4](https://github.com/flahercc/Youtarr/issues/4).
...
- `docs/CONFIG.md`: `config/config.json` schema. Real field names include `plexApiKey`, `plexYoutubeLibraryId`, `plexSubfolderLibraryMappings`, `plexIP`/`plexPort`/`plexViaHttps`/`plexUrl`, `channelAutoDownload`, `channelDownloadFrequency` (cron), `channelFilesToDownload`, `channelScanEnabled`/`channelScanTime` (daily new-videos scan schedule, "HH:MM" server-local), `channelScanVideoLimit` (videos checked per channel tab during that scan), `preferredResolution`, `videoCodec`, `defaultSubfolder`, plus groups for SponsorBlock, notifications (`appriseUrls`), download performance, and auto-removal. Legacy `cronSchedule` is auto-migrated to `channelDownloadFrequency` on startup.
